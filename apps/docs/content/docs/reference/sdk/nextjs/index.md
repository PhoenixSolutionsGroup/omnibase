...
The SDK provides comprehensive authentication support through Ory Kratos integration. Access session data on the server using [`getServerSession()`](Auth.md), handle authentication flows with [`FlowRouter`](_media/flow-router.ts), and protect routes with middleware.
