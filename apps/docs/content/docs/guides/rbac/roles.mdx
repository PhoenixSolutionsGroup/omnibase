...
result, _, err := tenantsClient.CreateRole(ctx).CreateRoleRequest(omnibase.CreateRoleRequest{
    RoleName: "developer",
    Permissions: []string{
        "tenant#can_view_users",
        "tenant#can_create_api_keys",
...
```go
result, _, err := tenantsClient.ListRoles(ctx).Execute()
for _, role := range result.Data.Roles {
...
```go
_, _, err := tenantsClient.UpdateRole(ctx, roleId).UpdateRoleRequest(omnibase.UpdateRoleRequest{
    Permissions: []string{
        "tenant#can_view_users",
...
```go
_, _, err := tenantsClient.DeleteRole(ctx, roleId).Execute()
```
...
```go
_, _, err := tenantsClient.CreateInvite(ctx).CreateTenantUserInviteRequest(omnibase.CreateTenantUserInviteRequest{
    Email:    "teammate@example.com",
    Role:     "developer",
    InviteUrl: "https://app.example.com/auth/onboarding",
}).Execute()
...
```go
_, _, err := tenantsClient.UpdateTenantUserRole(ctx).UpdateTenantUserRoleRequest(omnibase.UpdateTenantUserRoleRequest{
    Role:   "admin",
    UserId: targetUserId,
}).Execute()
```
...
```go
func setupTenantRoles(ctx context.Context, tenantsClient *omnibase.V1TenantsAPIService) error {
    // Owner role is created automatically
