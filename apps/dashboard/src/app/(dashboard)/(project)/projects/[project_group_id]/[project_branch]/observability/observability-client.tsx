...
function buildEmbedURL(
  ...
  const params = new URLSearchParams({
    orgId: "1",
    timezone: "browser",
    "var-project_group_id": projectGroupId,
    kiosk: "true",
  });
...
export function ObservabilityClient({
  ...
  const embedURL = useMemo(() => {
    ...
    return buildEmbedURL(dashboardUID, projectGroupId, dashboard.services);
  }, [dashboardUID, projectGroupId]);
    ...
    <div className="flex flex-col h-full p-6 space-y-4">
      ...
      <div className="flex gap-2 flex-wrap items-center">
        {DASHBOARDS.map((dashboard) => (
          <a
            key={dashboard.uid}
            href={`/projects/${projectGroupId}/${projectBranch}/observability?dashboard=${dashboard.uid}`}
