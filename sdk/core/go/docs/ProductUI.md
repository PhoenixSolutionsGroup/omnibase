# ProductUI

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**DisplayName** | Pointer to **string** | Display name for UI | [optional] 
**Tagline** | Pointer to **string** | Short tagline | [optional] 
**Features** | Pointer to **[]string** | List of product features | [optional] 
**Badge** | Pointer to **string** | Badge text (e.g., \&quot;Popular\&quot;, \&quot;Best Value\&quot;) | [optional] 
**CtaText** | Pointer to **string** | Call-to-action button text | [optional] 
**Highlighted** | Pointer to **bool** | Whether to highlight this product | [optional] 
**SortOrder** | Pointer to **int32** | Display order for sorting | [optional] 

## Methods

### NewProductUI

`func NewProductUI() *ProductUI`

NewProductUI instantiates a new ProductUI object
This constructor will assign default values to properties that have it defined,
and makes sure properties required by API are set, but the set of arguments
will change when the set of required properties is changed

### NewProductUIWithDefaults

`func NewProductUIWithDefaults() *ProductUI`

NewProductUIWithDefaults instantiates a new ProductUI object
This constructor will only assign default values to properties that have it defined,
but it doesn't guarantee that properties required by API are set

### GetDisplayName

`func (o *ProductUI) GetDisplayName() string`

GetDisplayName returns the DisplayName field if non-nil, zero value otherwise.

### GetDisplayNameOk

`func (o *ProductUI) GetDisplayNameOk() (*string, bool)`

GetDisplayNameOk returns a tuple with the DisplayName field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetDisplayName

`func (o *ProductUI) SetDisplayName(v string)`

SetDisplayName sets DisplayName field to given value.

### HasDisplayName

`func (o *ProductUI) HasDisplayName() bool`

HasDisplayName returns a boolean if a field has been set.

### GetTagline

`func (o *ProductUI) GetTagline() string`

GetTagline returns the Tagline field if non-nil, zero value otherwise.

### GetTaglineOk

`func (o *ProductUI) GetTaglineOk() (*string, bool)`

GetTaglineOk returns a tuple with the Tagline field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetTagline

`func (o *ProductUI) SetTagline(v string)`

SetTagline sets Tagline field to given value.

### HasTagline

`func (o *ProductUI) HasTagline() bool`

HasTagline returns a boolean if a field has been set.

### GetFeatures

`func (o *ProductUI) GetFeatures() []string`

GetFeatures returns the Features field if non-nil, zero value otherwise.

### GetFeaturesOk

`func (o *ProductUI) GetFeaturesOk() (*[]string, bool)`

GetFeaturesOk returns a tuple with the Features field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetFeatures

`func (o *ProductUI) SetFeatures(v []string)`

SetFeatures sets Features field to given value.

### HasFeatures

`func (o *ProductUI) HasFeatures() bool`

HasFeatures returns a boolean if a field has been set.

### GetBadge

`func (o *ProductUI) GetBadge() string`

GetBadge returns the Badge field if non-nil, zero value otherwise.

### GetBadgeOk

`func (o *ProductUI) GetBadgeOk() (*string, bool)`

GetBadgeOk returns a tuple with the Badge field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetBadge

`func (o *ProductUI) SetBadge(v string)`

SetBadge sets Badge field to given value.

### HasBadge

`func (o *ProductUI) HasBadge() bool`

HasBadge returns a boolean if a field has been set.

### GetCtaText

`func (o *ProductUI) GetCtaText() string`

GetCtaText returns the CtaText field if non-nil, zero value otherwise.

### GetCtaTextOk

`func (o *ProductUI) GetCtaTextOk() (*string, bool)`

GetCtaTextOk returns a tuple with the CtaText field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetCtaText

`func (o *ProductUI) SetCtaText(v string)`

SetCtaText sets CtaText field to given value.

### HasCtaText

`func (o *ProductUI) HasCtaText() bool`

HasCtaText returns a boolean if a field has been set.

### GetHighlighted

`func (o *ProductUI) GetHighlighted() bool`

GetHighlighted returns the Highlighted field if non-nil, zero value otherwise.

### GetHighlightedOk

`func (o *ProductUI) GetHighlightedOk() (*bool, bool)`

GetHighlightedOk returns a tuple with the Highlighted field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetHighlighted

`func (o *ProductUI) SetHighlighted(v bool)`

SetHighlighted sets Highlighted field to given value.

### HasHighlighted

`func (o *ProductUI) HasHighlighted() bool`

HasHighlighted returns a boolean if a field has been set.

### GetSortOrder

`func (o *ProductUI) GetSortOrder() int32`

GetSortOrder returns the SortOrder field if non-nil, zero value otherwise.

### GetSortOrderOk

`func (o *ProductUI) GetSortOrderOk() (*int32, bool)`

GetSortOrderOk returns a tuple with the SortOrder field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetSortOrder

`func (o *ProductUI) SetSortOrder(v int32)`

SetSortOrder sets SortOrder field to given value.

### HasSortOrder

`func (o *ProductUI) HasSortOrder() bool`

HasSortOrder returns a boolean if a field has been set.


[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


